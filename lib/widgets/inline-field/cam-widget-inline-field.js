  ...
  function(
    ...
    return {
      ...
      link: function(scope, element) {
        ...
        scope.selectNextInlineField = function() {
          var allFields = $("[cam-widget-inline-field][type='text']");
          for(var i = 0; i < allFields.length-1; i++) {
            if(allFields[i] === element[0]) {
              $timeout(function(){$(allFields[i+1]).find(".view-value").click();});
              return;
            }
          }
          // take the first one
          $timeout(function(){$(allFields[0]).find(".view-value").click();});
        };
