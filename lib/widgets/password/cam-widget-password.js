...
module.exports = ['camAPI', 'debounce', '$translate', '$sce',
    ...
    return {
      ...
      link: function($scope) {
        ...
        function createRules() {
          passwordPolicyProvider.get().then(function(res) {
            ...
            res.rules.forEach(function(rule) {
              policyText += '<li>' + $translate.instant(rule.placeholder, rule.parameter) + '</li>';
            });
            policyText += '</ul>';
        ...
        var createRestCall = debounce(function() {
          ...
          passwordPolicyProvider.validate($scope.password, function(err, res) {
            if(err) {
              $scope.loadingState = 'NOT_OK';
              $scope.tooltipText = $sce.trustAsHtml($translate.instant('PASSWORD_POLICY_TOOLTIP_ERROR'));
              return;
            }
            if(res.valid) {
              ...
              $scope.tooltipText = $translate.instant('PASSWORD_POLICY_TOOLTIP_PARTIAL');
              $scope.tooltipText += '<ul>';
              res.rules.forEach(function(rule) {
                if(!rule.valid) {
                  $scope.tooltipText += '<li>' + $translate.instant(rule.placeholder, rule.parameter) + '</li>';
                }
              });
              $scope.tooltipText += '</ul>';
