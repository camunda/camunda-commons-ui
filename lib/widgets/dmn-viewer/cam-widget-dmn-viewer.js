...
var migrateDiagram = require('../../../dmn-js/index.js').migrateDiagram;
var Viewer = require('../../../dmn-js/index.js').NavigatedViewer;
var Modeler = require('../../../dmn-js/index.js').Modeler;
  ...
  return {
    ...
    link: function($scope, $element) {
      ...
      $scope.control.highlightElement = function(id) {
        if (canvas && viewer.getActiveViewer().get('elementRegistry').get(id)) {
      ...
      $scope.control.getElements = function(filter) {
        if (canvas) {
          return viewer.getActiveViewer().get('elementRegistry').filter(filter);
        }
      };
      ...
      $scope.$watch('xml', function(newValue) {
        if (newValue) {
          migrateDiagram(newValue).then(function(dmn13Xml) {
            xml = dmn13Xml;
            renderTable();
          });
        }
      ...
      $scope.zoomIn = function() {
        viewer.getActiveViewer().get('zoomScroll').zoom(1, {
      ...
      function selectView() {
        if ($scope.table) {
          ...
          var view = viewer.getViews().filter(function(view) {
            return (angular.isString($scope.table) &&
