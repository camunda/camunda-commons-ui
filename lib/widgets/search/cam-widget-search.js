      ...
      link: function($scope, element) {
        ...
        $scope.switchVariableCases = function() {
          if($scope.isVariableCaseHandlingActive) {
            // Cycle through all possible variations
            var options = ['none', 'names', 'values', 'all'];
            $scope.variableCaseHandling = options[(options.indexOf( $scope.variableCaseHandling) + 1) % options.length];
          }
        };
        ...
        var handleSearchesUpdate = function() {
          ...
          if ($scope.isVariableCaseHandlingActive) {
            $location.search('variableCaseHandling', $scope.variableCaseHandling);
          }
        ...
        $scope.$watch('[searches, matchAny, variableCaseHandling]', handleSearchesUpdate, true);
