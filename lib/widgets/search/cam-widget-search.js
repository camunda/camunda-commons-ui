  ...
  function($timeout, $location, searchService, widgetLocalConf) {
    ...
    return {
      ...
      link: function($scope, element) {
        ...
        $scope.isVariableCaseHandlingActive =
          typeof $scope.variableCaseHandling !== 'undefined';

        if ($scope.isVariableCaseHandlingActive) {
          $scope.caseHandeling = {};
          $scope.caseHandeling.ignoreValues =
            $scope.variableCaseHandling === 'values' ||
            $scope.variableCaseHandling === 'all';
          $scope.caseHandeling.ignoreNames =
          ...
          angular.forEach($scope.searches, function(value) {
            if (
              [
                ...
                'taskVariables',
                'processVariables'
              ].indexOf(value.type.value.key) > -1
            ) {
        ...
        var handleSearchesUpdate = function() {
          ...
          if ($scope.isVariableCaseHandlingActive) {
            ...
            $scope.variableCaseHandling =
              options[
                !!$scope.caseHandeling.ignoreNames +
                  !!$scope.caseHandeling.ignoreValues * 2
              ];
