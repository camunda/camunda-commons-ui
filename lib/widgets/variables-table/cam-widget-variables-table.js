  ...
  function(
    ...
    return {
     ...
     /*   $scope.headers = ($scope.headers || {
       ...
       // PASS Columns from the environment, as history works differently than runtime.

        $scope.headColumns = [
          { class: 'name',  request: 'variableName', sortable: true, content: $translate.instant('CAM_WIDGET_VARIABLES_TABLE_NAME')},
          { class: 'type',  request: 'variableType', sortable: true, content: $translate.instant('CAM_WIDGET_VARIABLES_TABLE_TYPE')},
          { class: 'value', request: 'value',        sortable: false, content: $translate.instant('CAM_WIDGET_VARIABLES_TABLE_VALUE')}
        ];

        // Default Sorting
        $scope.defaultSort = { sortBy: 'variableName', sortOrder: 'desc' };

        // Array of header Names/class
        $scope.headerNames = [];
        $scope.headColumns.forEach(function(column) {
          $scope.headerNames.push(column.class);
        });

        $scope.editable = ($scope.editable ||  $scope.headerNames);
        ...
        */
        ...
        $scope.hasEditDialog = function(type) {
          return type && ['object', 'string', 'json', 'xml'].indexOf(type.toLowerCase()) > -1;
        };
