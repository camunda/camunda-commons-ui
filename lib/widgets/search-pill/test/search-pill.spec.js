...
var pageUrl = 'http://localhost:' + pkg.gruntConfig.connectPort +
              '/lib/widgets/search-pill/test/search-pill.spec.html';
...
describe('Search Pill', function() {
  ...
  describe('Basic', function() {
    ...
    it('should allow text input', function() {
      ...
      expect(valueField.inputField().getAttribute('ng-model')).to.eventually.eql(browser.driver.switchTo().activeElement().getAttribute('ng-model'));
    });
  ...
  describe('Date Enforced', function() {
    ...
    it('should format a date', function() {
      ...
      expect(valueField.text()).to.eventually.eql('January 15, 2015 12:46 PM');
    });
  });
  ...
  describe('Extended', function() {
    ...
    it('name field should be text input', function() {
      ...
      // workaround to check focus
      // see: http://stackoverflow.com/a/22756276
      expect(pill.nameField().inputField().getAttribute('ng-model')).to.eventually.eql(browser.driver.switchTo().activeElement().getAttribute('ng-model'));
    });
