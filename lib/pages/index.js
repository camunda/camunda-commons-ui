  ...
  var ResponseErrorHandlerInitializer = [
    '$rootScope', '$location', 'Notifications',   'AuthenticationService', 'shouldDisplayAuthenticationError', '$translate',
    function($rootScope, $location, Notifications, AuthenticationService, shouldDisplayAuthenticationError, $translate) {
     ...
     */
      function handleHttpError(event, error) {
        ...
        case 0:
          addError({ status: $translate.instant('PAGES_STATUS_REQUEST_TIMEOUT'), message: $translate.instant('PAGES_MSG_REQUEST_TIMEOUT') });
          break;
        ...
        case 403:
          if (data.type == 'AuthorizationException') {
            var message;
            if (data.resourceId) {
              message = $translate.instant('PAGES_MSG_ACCESS_DENIED_RESOURCE_ID', {
                permissionName: data.permissionName.toLowerCase(),
                resourceName: data.resourceName.toLowerCase(),
                resourceId: data.resourceId
              });
            } else {
              var missingAuths = data.missingAuthorizations.map(function(missingAuth) {
                return (
                  '\'' +
        ...
        case 404:
          if (shouldDisplayAuthenticationError()) {
            addError({ status: $translate.instant('PAGES_STATUS_NOT_FOUND'), message: $translate.instant('PAGES_MSG_NOT_FOUND') });
          }
          break;
        default:
          addError({
            status: $translate.instant('PAGES_STATUS_COMMUNICATION_ERROR'),
            message: $translate.instant('PAGES_MSG_COMMUNICATION_ERROR', {status: status})
          });
        }
      }
    ...
    function($scope, $http, $location, $window, Uri, Notifications, $translate) {
      ...
      $http.get(Uri.appUri('engine://engine/')).then(function(response) {
        ...
        if (!$scope.currentEngine) {
          Notifications.addError({ status: $translate.instant('PAGES_STATUS_NOT_FOUND'), message: $translate.instant('PAGES_MSG_ENGINE_NOT_EXISTS'), scope: $scope });
          $location.path('/');
        }
      }).catch(angular.noop);
  ...
  var AuthenticationController = [
    '$scope', '$window', '$cacheFactory', '$location', 'Notifications', 'AuthenticationService',
    function($scope, $window, $cacheFactory, $location, Notifications, AuthenticationService) {
